...
public class AddOwner {
    public static void addOwner(AreaProtect areaProtect, Player player,
            String[] args) {
        ...
        if (!(args.length >= 2)) {
            PlayerLogger.message(player,
                    language.getMessage("Player.AddOwner.Usage"));
            return;
        }
        ...
        if (args.length == 2) {
            ...
            if (regions.size() == 0) {
                PlayerLogger.message(player,
                        language.getMessage("Player.AddOwner.Not-In-Area"));
                return;
            }
            ...
            for (ProtectedRegion region : regions) {
                if (!region.isOwner(localPlayer)
                        && !player
                                .hasPermission("areaprotect.ap.addowner.bypass.owner")) {
                    PlayerLogger.message(player,
                            language.getMessage("Player.AddOwner.Not-Owner"));
                    return;
                }

                if (!region.getId().contains("areaprotect")) {
                    PlayerLogger.message(player, language
                            .getMessage("Player.AddOwner.Not-Areaprotect"));
                    return;
                }
                ...
                if (!areaProtect.getServer().getOfflinePlayer(memberName)
                    ...
                    PlayerLogger.message(player, language.getMessage(
                            "Player.AddOwner.No-Player", new String[] {
                                    "%player%", memberName }));
                    return;
                }
                ...
                if (owners.contains(memberName)) {
                    PlayerLogger.message(player, language.getMessage(
                            "Player.AddOwner.Already-Owner", new String[] {
                                    "%player%", memberName }));
                    return;
                }

                owners.addPlayer(memberName);
            ...
            if (region == null) {
                PlayerLogger.message(player, language.getMessage(
                        "Player.AddOwner.Invalid-Name", new String[] {
                                "%Area_Name%", args[1] }));
                return;
            }
            ...
            if (!areaProtect.getServer().getOfflinePlayer(memberName)
                    .hasPlayedBefore()) {
                PlayerLogger.message(player, language.getMessage(
                        "Player.AddOwner.No-Player", new String[] { "%player%",
                                memberName }));
                return;
            }
